...
if __name__ == "__main__":
    ...
    # prepare storage
    # class_prec = {cfg["name"]: [] for cfg in CONFIGS}
    ...
    for cfg in CONFIGS:
        ...
        for cls in range(num_classes):
            ...
            # 1) y_true: 1 if image contains class `cls`
            y_true = [int(cls in gtc) for gtc in gt_classes]

            y_scores = []
            for fn in file_names:
                frame = cv2.imread(fn)
                b, c, s, _ = det.post_process(det.predict(frame))
                # collect all confidences for this class
                cls_scores = [score for label, score in zip(c, s) if label == cls]
                y_scores.append(max(cls_scores) if cls_scores else 0.0)

            # for each image
            # for fn, gtc in zip(file_names, gt_classes):
            #     # 1 = contains at least one GT of this class
            #     y_true.append(int(cls in gtc))

            #     # run model & pick max score for this class
            #     frame = cv2.imread(fn)
            #     b, c, s, _ = det.post_process(det.predict(frame))
            #     scores_cls = [score for lab, score in zip(c, s) if lab == cls]
            #     y_scores.append(max(scores_cls) if scores_cls else 0.0)

            # 3) compute AP@0.5 via sklearn
            ap = average_precision_score(y_true, y_scores)
            ap_results[cfg["name"]].append(ap)
        print(f" → Done {cfg['name']}\n")
#####################
            # # binarize at threshold
            # y_pred = [1 if sc >= threshold else 0 for sc in y_scores]
            # # compute precision (zero_division=0 avoids NaNs if no preds)
            # prec = precision_score(y_true, y_pred, zero_division=0)
            # class_prec[cfg["name"]].append(prec)
    ...
    # ax.bar(x - width/2, class_prec[CONFIGS[0]["name"]], width, label=CONFIGS[0]["name"])
    # ax.bar(x + width/2, class_prec[CONFIGS[1]["name"]], width, label=CONFIGS[1]["name"])
    ...
    # ax.set_ylabel(f"Precision @ {threshold}")
    # ax.set_title(f"Per-class Precision @ {threshold}: Model 1 vs Model 2")
    ax.legend()
    plt.tight_layout()

    ts = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    out_path = f"per_class_AP_comparison_{ts}.png"
    # out_path = f"per_class_precision_{threshold}_{ts}.png"
    plt.savefig(out_path)
    print(f"Saved per-class AP chart to {out_path}")
