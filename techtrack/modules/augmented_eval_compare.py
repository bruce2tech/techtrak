...
def evaluate_single_image(args):
    ...
    img = apply_augmentation(cv2.imread(fn), aug_type)
...
if __name__ == "__main__":
    ...
    all_model_scores = {}
    AUGMENTATIONS = ["none", "gaussian", "flip", "brightness"]

    for aug_type in AUGMENTATIONS:
        print(f"\n🔍 Evaluating with augmentation: {aug_type.upper()}\n")
    # For each model, spin up a pool and evaluate all images in parallel
        for cfg in CONFIGS:
            ...
            tasks = [(fn, cfg, aug_type) for fn in file_names]
            ...
            with ProcessPoolExecutor(max_workers=os.cpu_count()) as exe:
                ...
                for future in as_completed(futures):
                    ...
                    key = f"{cfg['name']}__{aug_type}"
                    if key not in all_model_scores:
                        all_model_scores[key] = {}
                    all_model_scores[key][fn] = max_scores
    ...
    num_classes = len(class_names)
    metrics_data = {}
    for full_model_key, img_scores in all_model_scores.items():
        model_name, aug_type = full_model_key.split("__")
        key = f"{model_name}__{aug_type}"
        model_data = {}
        ...
        metrics_data[key] = model_data

    # save everything
    out_name = f"raw_detection_data_augstudy_{time.strftime('%Y%m%d_%H%M%S')}.pkl"
